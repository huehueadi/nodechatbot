...
// import { saveToJson, saveToTxtWithSizeLimit, scrapeWebsite, scrapePage } from "./scrapingLogic.js";
// import { uploadToS3 } from "../utils/s3Utils.js";

// export const scrapeAndUploadService = async ({ url, choice, userId, bucketName }) => {
...
//     if (choice === 'all') {
//       const { allData: scrapedData, totalLinks: links, totalPdfs: pdfs } = await scrapeWebsite(browser, url);
//       allData = scrapedData;
//       totalLinks = links;
//       totalPdfs = pdfs;
//     } else {
...
//     const outputBaseFilename = `scraped_data_${timestamp}`;
//     const jsonFilePath = `${outputBaseFilename}.json`;

//     saveToJson(allData, jsonFilePath);
//     saveToTxtWithSizeLimit(allData, outputBaseFilename);

//     // Upload JSON to S3
//     const s3Key = `${userId}/${jsonFilePath}`;
//     await uploadToS3(bucketName, s3Key, jsonFilePath);
...
//     };
//   } catch (error) {
//     console.error('Error in scrapeAndUploadService:', error);
//     throw error;
//   } finally {
//     await browser.close();
//   }
